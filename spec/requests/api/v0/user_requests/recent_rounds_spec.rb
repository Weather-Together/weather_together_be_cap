...
RSpec.describe "User Recent Rounds" do
  ...
  xit "get request for /users/:user_id/recent_rounds (info: 3 most recents public rounds with votes, vote content, placement in round)", :vcr do
    ...
    expect(json_response["data"][0]["id"]).to eq(@round4.id.to_s)
    ...
    expect(json_response["data"][1]["id"]).to eq(@round3.id.to_s)
