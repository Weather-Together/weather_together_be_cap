...
RSpec.describe "User Update Profile Endpoint" do
  ...
  describe "sad path" do
    ...
    it "returns an error if attribute being updated is left blank", :vcr do
      ...
      expect(response_data[:detail]).to eq("Username cannot be blank")
    end

    it "returns an error if username already exists", :vcr do
      test_data
      ...
      expect(response_data[:detail]).to eq("Username already exists")
    end
  end
end